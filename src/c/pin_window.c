...
static char* selection_handle_get_text(int index, void *context) {
  ...
  if (index == 0)
  {
    ...
    sizeof(pin_window->field_buffs), "%d",
  ...
  pin_window->info->tm_mday = pin_window->pin.digits[0];
  pin_window->info->tm_hour = pin_window->pin.digits[1];
  pin_window->info->tm_min = pin_window->pin.digits[2];
  strftime(pin_window->pin.date,24,"%x %H:%M:00",  pin_window->info );
  text_layer_set_text(pin_window->sub_text, pin_window->pin.date);
  }
...
static void selection_handle_inc(int index, uint8_t clicks, void *context) {
  ...
  {
    case 0:
      if(pin_window->pin.digits[index] > PIN_WINDOW_MAX_VALUE_DAY) 
      {
          pin_window->pin.digits[index] = 1;
          pin_window->info->tm_mon += 1;
      }      
      break;
    case 1:
      if(pin_window->pin.digits[index] > PIN_WINDOW_MAX_VALUE_HOUR)
        pin_window->pin.digits[index] = 0;
        break;
    case 2:
      if(pin_window->pin.digits[index] > PIN_WINDOW_MAX_VALUE_MINUTE)
        pin_window->pin.digits[index] = 0;
      break;
  }
}
...
static void selection_handle_dec(int index, uint8_t clicks, void *context) {
  ...
  {
    case 0:
      if(pin_window->pin.digits[index] < 1) 
      {
        pin_window->pin.digits[index] = PIN_WINDOW_MAX_VALUE_DAY;
        pin_window->info->tm_mon -= 1;
      }
      break;
    case 1:
      if(pin_window->pin.digits[index] < 0)
        pin_window->pin.digits[index] = PIN_WINDOW_MAX_VALUE_HOUR;
        break;
    case 2:
      if(pin_window->pin.digits[index] < 0)
        pin_window->pin.digits[index] = PIN_WINDOW_MAX_VALUE_MINUTE;
      break;
  }
}
...
PinWindow* pin_window_create(PinWindowCallbacks callbacks) {
  ...
  if (pin_window) {
    ...
    if (pin_window->window) {
      ...
      //Time
      time(&pin_window->rawtime);
      pin_window->info = localtime(&pin_window->rawtime);
      pin_window->pin.digits[0] =  pin_window->info->tm_mday;
      pin_window->pin.digits[1] =  pin_window->info->tm_hour;
      pin_window->pin.digits[2] =  pin_window->info->tm_min;
      ...
      // Main TextLayer
      const GEdgeInsets main_text_insets = {.top = 25};
      pin_window->main_text = text_layer_create(grect_inset(bounds, main_text_insets));
      text_layer_set_text(pin_window->main_text, "Enter the reminder date to continue");
      ...
      //text_layer_set_text(pin_window->sub_text, "D-Day HH:mm");
      strftime(pin_window->pin.date,24,"%x - %H:%M",  pin_window->info );
      text_layer_set_text(pin_window->sub_text, pin_window->pin.date);
